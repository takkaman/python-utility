setenv duo_dir /u/phyan/utility/duo
echo "Usage Flow:"
...
proc get_qor args {
    exec sed "s/TITLE/Report for cell/g" \
        /u/phyan/utility/duo/head.html  > qor_info.html
    ...
    set qor [exec /u/phyan/utility/duo/qor2.pl icc2_output.txt ]
...
proc duo args {
#    set duo_dir [sh cat /u/phyan/utility/duo/duo_dir]
...
#    exec mkdir duo_work
    ...
    if {[info exist option_input(-net)]} {
        set this_net [get_nets -quiet $option_input(-net)]
        if {$this_net == ""} {
            return
        } else {
            set net_name [get_object_name $this_net]
        }
    ...
    if {[info exist option_input(-density)]} {                                                                 
       set is_density 1;                                                                                      
       exec mkdir duo_work_${date}_density
       set duo_work_dir duo_work_${date}_density
    }     

    if {[info exist option_input(-congestion)]} {
       set is_cong 1;
       exec mkdir duo_work_${date}_congestion
       set duo_work_dir duo_work_${date}_congestion
    }
    ...
    foreach one_design $all_designs {
        if {[regexp {(^[A-Z]+\d*)([a-z]*)\.design$}  [set one_design] all step pfix] } {
            if {[lsearch $all_stage $step] != -1}  {
                 lappend checkpoints $one_design
                 echo "$step$pfix" >> $duo_work_dir/checkpoints
            }
        }
    }
...
#    sh rm -f *.job
    ...
    while {1} {
        ...
        foreach one_check $checkpoints {
            if {[file exist ${duo_work_dir}/$one_check/$clib.$one_check.info]}  {
                incr i
            } else {
...
#            echo "$step" [sh cat $clib.$one_check.info]
            puts [format "%*s%s" $w1 $step [sh cat ${duo_work_dir}/$one_check/$clib.$one_check.info]]
        }
        ...
        exec sed "s/TITLE/Report for $cell_name/g; "  /u/phyan/utility/duo/head.html  >  ${duo_work_dir}/cell_info.html
        ...
        foreach one_check $checkpoints {
            echo "<tr>" >>  ${duo_work_dir}/cell_info.html
            regexp {(\S+)\.design} $one_check all step
            if {[file exist ${duo_work_dir}/$one_check/$clib.$one_check.cell.snapshot.gif]} {
                echo "<td><a href=\"$one_check/$clib.$one_check.cell.snapshot.gif\"> $step</a> </td>" >>  ${duo_work_dir}/cell_info.html
            } else {
                echo "<td> $step </td>" >>  ${duo_work_dir}/cell_info.html
            }
            set cell_info [sh cat ${duo_work_dir}/$one_check/$clib.$one_check.info]
    ...
    if {$path_from != "" || $path_through != "" || $path_to != ""} {
        ...
        foreach one_check $checkpoints {
            ...
            regexp {(\S+)\.design} $one_check all step
            puts [format "%*s%s" $w1 $step [sh cat ${duo_work_dir}/$one_check/$clib.$one_check.info]]
            if {[file exist ${duo_work_dir}/$one_check/$clib.$one_check.path.snapshot.gif]} {
                echo "<td><a href=\"$one_check/$clib.$one_check.path.snapshot.gif\"> $step</a> </td> <td> [sh cat ${duo_work_dir}/$one_check/$clib.$one_check.info] </td>" >> ${duo_work_dir}/path.html
                echo "</tr>" >> ${duo_work_dir}/path.html
            } else {
                echo "<td> $step </td> <td> [sh cat ${duo_work_dir}/$one_check/$clib.$one_check.info] </td>" >> ${duo_work_dir}/path.html
                echo "</tr>" >> ${duo_work_dir}/path.html
            }
...
#            echo "$step" [sh cat $clib.$one_check.info]
        ...
        exec sed "s/TITLE/Report for net/g" \
         /u/phyan/utility/duo/head.html  > ${duo_work_dir}/net_info.html
        ...
        foreach one_check $checkpoints {
            echo "<tr>" >> ${duo_work_dir}/net_info.html
            regexp {(\S+)\.design} $one_check all step
            if {[file exist ${duo_work_dir}/$one_check/$clib.$one_check.net.snapshot.gif]} {
                echo "<td><a href=\"$one_check/$clib.$one_check.net.snapshot.gif\"> $step</a> </td>" >> ${duo_work_dir}/net_info.html
            }  else {
                echo "<td> $step </td>" >> ${duo_work_dir}/net_info.html
            }
            set net_info [sh cat ${duo_work_dir}/$one_check/$clib.$one_check.info]
     ...
     if {$is_density}  {                                                                                        
         exec sed "s/TITLE/Report for net/g" \
          /u/phyan/utility/duo/head.html  >  ${duo_work_dir}/density_info.html                  
         ...
         foreach one_check $checkpoints {
             echo "<tr>" >>  ${duo_work_dir}/density_info.html
             regexp {(\S+)\.design} $one_check all step
             if {[file exist ${duo_work_dir}/$one_check/$clib.$one_check.density.snapshot.gif]} {
                 echo "<td><a href=\"$one_check/$clib.$one_check.density.snapshot.gif\"> $step</a> </td>" >>  ${duo_work_dir}/density_info.html
             }  else {
                 echo "<td> $step </td>" >>  ${duo_work_dir}/density_info.html
             }
             set net_info [sh cat ${duo_work_dir}/$one_check/$clib.$one_check.info]
     ...
     if {$is_cong}  {
         exec sed "s/TITLE/Report for net/g" \
          /u/phyan/utility/duo/head.html  > ${duo_work_dir}/cong_info.html
         ...
         foreach one_check $checkpoints {
             echo "<tr>" >> ${duo_work_dir}/cong_info.html
             regexp {(\S+)\.design} $one_check all step
             if {[file exist ${duo_work_dir}/$one_check/$clib.$one_check.congestion.snapshot.gif]} {
                 echo "<td><a href=\"$one_check/$clib.$one_check.congestion.snapshot.gif\"> $step</a> </td>" >> ${duo_work_dir}/cong_info.html
             }  else {
                 echo "<td> $step </td>" >> ${duo_work_dir}/cong_info.html
             }
             set net_info [sh cat ${duo_work_dir}/$one_check/$clib.$one_check.info]
